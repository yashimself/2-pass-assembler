                            ...
                            if (!(inst[0].equals("START") || inst[0].equals("END"))) {
                                ...
                                if (inst[1].contains(",")) {
                                    ...
                                    if (tempstring[0].equals(Registers[0])) {
                                        if (!tempstring[1].equals(Registers[0]) && !(isNumeric(tempstring[1]))) {
                                            if(tempstring[1].contains("=")){
                                                String[] lit = tempstring[1].split("'");
                                                //System.out.println("Here ! HEre! HEre!"+lit[1]);
                                                String litadd = t+"\t"+tempstring[1]+"\t"+LC;
                                                Literals.add(litadd);
                                                litindex[t] = lit[1];
                                                t+=1;
                                                String temp = "(" + Type[i] + "," + Opcode[i] + ")" + "(" + Registers.length + ")" + "(" + "L" + "," + lit[1] + ")";
                                ...
                                LCarr.add(LC);
                                LC += Size[8];
                                String getme = Literals.get(0);
                                String[] getmearr = getme.split("\t");
                                int thefirstpool = Integer.parseInt(getmearr[2]);
                                PoolTab.add("#"+getmearr[0]);
                                if(Literals.size()>1){                           //Pool Table allocation happpens here
                                    for (i=1; i<Literals.size(); i++){
                                        getme = Literals.get(i);
                                        getmearr = getme.split("\t");
                                        int nowLCis = Integer.parseInt(getmearr[2]);
                                        if (nowLCis != (thefirstpool+1))
                                            PoolTab.add("#"+getmearr[0]);
                                        thefirstpool = nowLCis;
                                    }
                                }
